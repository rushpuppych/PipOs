...
var PipOsFormBuilder = function() {
  ...
  //var validator = new PipValueValidator();
  ...
  this.setGuiData = function(objData) {
    setTimeout(function() {
      // TODO: this is not working after some time ?!!
  ...
  _private.eventManager = function() {
    // Submit Eventhandler
      ...
      if($(this).attr('data-close-modal') == "true") {
        $(this).closest('div.pip-modal').find('span.close_modal_btn').trigger('click');
      }
   ...
   */
    ...
    // Html Template
		var strHtml = '';
		strHtml += '<button type="button" class="btn btn-sm btn-success form-submit-btn" data-close-modal="{{close_modal}}" data-validate-form="{{validate_form}}">Save</button>';
